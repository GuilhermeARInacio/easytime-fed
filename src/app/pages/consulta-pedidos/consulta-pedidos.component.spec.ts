...
describe('ConsultaPedidosComponent', () => {
  ...
  beforeEach(async () => {
    pontoServiceSpy = jasmine.createSpyObj('PontoService', ['consultarPedidos']);
...
it('deve popular pedidos e exibir sucesso ao consultar com sucesso', (done) => {
  ...
  it('deve exibir erro de login expirado e chamar sair ao receber erro 401', (done) => {
    pontoServiceSpy.consultarPedidos.and.returnValue(throwError(() => ({ status: 401, error: 'Login expirado. Por favor, faça login novamente.' })));
  ...
  it('deve exibir erro interno ao receber erro 500', (done) => {
    pontoServiceSpy.consultarPedidos.and.returnValue(throwError(() => ({ status: 0, error: 'Desculpe, ocorreu um erro interno ao tentar consultar os pedidos. Tente novamente mais tarde.' })));

    component.consultar();

    setTimeout(() => {
      expect(component.error).toBe('Desculpe, ocorreu um erro interno ao tentar consultar os pedidos. Tente novamente mais tarde.');
      expect(popUpServiceSpy.abrirNotificacao).toHaveBeenCalled();
      done();
    }, 600);
  });
  ...
  it('deve exibir mensagem de nenhum pedido encontrado se erro incluir "Nenhum pedido"', (done) => {
    pontoServiceSpy.consultarPedidos.and.returnValue(throwError(() => ({ status: 400, error: 'Nenhum pedido encontrado' })));

    component.consultar();

    setTimeout(() => {
      expect(component.error).toBe('Desculpe, não existem pedidos com essas características.');
      expect(component.carregando).toBeFalse();
      done();
    }, 600);
  });
  ...
  //   component.formulario.get('tipo')?.setValue('#');
  ...
  //   expect(pontoServiceSpy.consultarPedidos).not.toHaveBeenCalled();
  // });
