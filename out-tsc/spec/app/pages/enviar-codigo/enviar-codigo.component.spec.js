...
describe('EnviarCodigoComponent', () => {
    ...
    it('deve enviar codigo com sucesso', () => {
        ...
        expect(routerSpy.navigate).toHaveBeenCalledWith(['/trocar-senha'], { state: { response: 'success-response', email: emailValue } });
    });
    it('deve retornar erro 401', () => {
        const component = getComponent();
        component.formulario.setValue({ email: 'test@example.com' });
        usuarioServiceSpy.enviarCodigo.and.returnValue(throwError(() => ({ status: 401, error: { message: 'Unauthorized' } })));
        component.enviarEmail();
        expect(component.carregando).toBeFalse();
    ...
    it('deve tratar outros tipos de erro', () => {
        const component = getComponent();
        component.formulario.setValue({ email: 'test@example.com' });
        usuarioServiceSpy.enviarCodigo.and.returnValue(throwError(() => ({ status: 400, error: 'Erro ao enviar código.' })));
        component.enviarEmail();
        expect(component.carregando).toBeFalse();
    ...
    it('deve definir mensagem de erro padrão se err.error.message estiver ausente', () => {
        const component = getComponent();
        component.formulario.setValue({ email: 'test@example.com' });
        usuarioServiceSpy.enviarCodigo.and.returnValue(throwError(() => ({ status: 400 })));
        component.enviarEmail();
        expect(component.carregando).toBeFalse();
    ...
    it('deve tratar erro 500', () => {
        const component = getComponent();
        component.formulario.setValue({ email: 'test@example.com' });
        usuarioServiceSpy.enviarCodigo.and.returnValue(throwError(() => ({ status: 500 })));
        component.enviarEmail();
        expect(component.carregando).toBeFalse();
